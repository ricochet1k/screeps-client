...
export default class ConstructedWall extends GameObject {
	constructor(obj) {
		super(obj);
	...
	preRender(timestamp, room) {
		...
		if (this.dirty) {
			...
			let ns = neighbors(obj.x, obj.y, clampXYf((x, y) => 
				!!room.layers['constructedWall'].grid.get(x, y)));
			// console.log('constructedWall', obj, ns);
			renderSquare(g, 0, 0, ns, 0x222222);
